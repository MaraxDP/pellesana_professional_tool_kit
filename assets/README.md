...
- `assets/video/pelle-sana-hero.mp4` — video centrale della hero (autoplay,
  muto, in loop). Finché il file non è presente, il riquadro mostra
  automaticamente il placeholder "Video in arrivo" (nessun errore in
  console).
- `assets/img/hero/hero-video-cover.jpg` — poster facoltativo mostrato prima
  che il video parta o mentre carica.
...
exobio-adrn-plus.jpg         dr-pen-ultima-a6.jpg
```

`exobio-adrn-plus.jpg` e `dr-pen-ultima-a6.jpg` sono due schede aggiunte con la
riclassificazione della sezione Attivi professionali (categorie Biorivitalizzanti
e Accessori complementari):
- `exobio-adrn-plus.jpg` è una composizione delle due foto ufficiali già presenti
  (Exobio Plus + ADRN Plus affiancate), a rappresentare il protocollo combinato.
- `dr-pen-ultima-a6.jpg` e i relativi contenuti testuali (funzione, descrizione,
  indicazioni) sono stati estratti dal Catalogo InLab Medical ufficiale
  (sezione "Accessori complementari").
Se in futuro sarà disponibile una foto ufficiale a risoluzione più alta per
uno dei due prodotti, sostituire il file mantenendo lo stesso nome.
...
training, Materiali marketing, Anamnesi.

- **Video training** ha una sorgente dati dedicata, l'array
  `videoTrainingItems` in `script.js` (copertina, titolo, descrizione,
  link YouTube incorporato, eventuale `downloadUrl`).
- Le altre 3 aree (Video introduttivi, Materiali marketing, Anamnesi) sono
  oggetti nell'array `formazioneItems`, con campo `area` che determina sotto
  quale tab compare. Per aggiornare un contenuto, modificare `titolo`,
...
`kit-social.zip`, `guida-anamnesi.pdf`). Il modulo di consenso nel tab
Anamnesi (`form-11`) punta volutamente allo stesso file della Libreria
materiali, `modulo-consenso.pdf` (vedi sotto): stesso titolo, stessa
descrizione, stesso link, per evitare due versioni diverse dello stesso
documento. Allo stesso modo, la Checklist di valutazione iniziale
(`form-12`) punta volutamente allo stesso file della Scheda Anamnesi
Cliente della Libreria materiali, `scheda-anamnesi-cliente.pdf`.
...
(`brochure.pdf`, `listino.pdf`, `modulo-consenso.pdf`,
`scheda-anamnesi-cliente.pdf`, `locandina.pdf`). I filtri ammessi in questa sezione sono solo cinque:
...
- `modulo-consenso.pdf` — **già presente** (voce `doc-03`, categoria
  "Moduli"): modulo di consenso informato per trattamento estetico, dati
  anamnestici e utilizzo di fotografie/video. È un modello operativo da
  personalizzare con i dati del centro e sottoporre a verifica privacy/legale
  prima dell'uso (vedi avvertenza nell'ultima pagina del PDF stesso).
